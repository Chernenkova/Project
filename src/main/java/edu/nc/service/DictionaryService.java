...
public class DictionaryService {
    ...
    public ResponseEntity<CardWrapper[]> delete(CardWrapper[] array) {
        ...
        ChoosingTranslationTaskWrapper wrapper = JsonClassParser.getInstance().getObject(taskEntity.getTask(), ChoosingTranslationTaskWrapper.class);
        wrapper.setCardsIds(removeIds(wrapper.getCardsIds(), getCardsIds(array)));
        taskEntity.setTask(JsonClassParser.getInstance().getBytes(wrapper));
        taskEntity = taskRepository.saveAndFlush(taskEntity);
        wrapper = JsonClassParser.getInstance().getObject(taskEntity.getTask(), ChoosingTranslationTaskWrapper.class);
    ...
    public ResponseEntity<CardWrapper[]> put(CardWrapper cardWrapper) {
        ...
        if (null == taskEntity) {
            ...
            byte[] bytes = JsonClassParser.getInstance().getBytes(task);
            taskEntity = new TaskEntity(GeneralSettings.DICTIONARY_TYPE, bytes, null, user, "", 0, 0);
        }
        long newId = putWordToDB(cardWrapper);
        ChoosingTranslationTaskWrapper taskWrapper = JsonClassParser.getInstance().getObject(taskEntity.getTask(), ChoosingTranslationTaskWrapper.class);
        ...
        taskEntity.setTask(JsonClassParser.getInstance().getBytes(taskWrapper));
        //update in DB
        taskEntity = taskRepository.saveAndFlush(taskEntity);
        return new ResponseEntity<>(getDictionary(JsonClassParser.getInstance().getObject(taskEntity.getTask(), ChoosingTranslationTaskWrapper.class).getCardsIds()), HttpStatus.OK);
    }
